#!/usr/bin/env python3
"""
Add A records to Hostinger DNS Zone (v2 - FIXED)
Official Docs: https://github.com/hostinger/api-python-sdk
Bug Fix: DNSV1ZoneUpdateRequestZoneInner requires 'name' and 'type' fields
"""
import os
...
try:
    ...
    # The zone also needs name, type fields (not just domain and records)
    # Try different values to understand the structure
    # Looking at the fields: name, type, records, ttl
    
    zone = hostinger_api.DNSV1ZoneUpdateRequestZoneInner(
        name=domain,           # Zone name = domain?
        type="zone",           # Type = "zone"?
        records=[record_root, record_www]
    )
    print(f'✅ Zone created with 2 A records')
    print(f'   Zone name: {domain}')
